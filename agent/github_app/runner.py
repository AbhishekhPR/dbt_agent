...
class PullRequestReviewRunner:
    ...
    def run(self, event, client):
        ...
        try:
            response = self._run_claimed(event, client)
        except Exception:
    ...
    def _run_claimed(self, event, client):
        ...
        manifest_content = client.get_file(owner, repository, config.manifest_path, event.head_sha)
        if manifest_content is None:
            return self._publish_neutral(
                ...
                message=(
                    f"Relium could not find `{config.manifest_path}` at the pull request head. "
                    "Generate and commit the dbt manifest or update `manifest_path` in `relium.yml`."
                ),
            )
        try:
        ...
        return self._publish(event, client, config.mode, result, status="reviewed")

    def _publish_neutral(self, event, client, mode, *, status, message):
        result = {"decision": "SKIPPED", "rendered": {"markdown": f"## Relium deployment review\n\n{message}"}}
        return self._publish(event, client, mode, result, status=status)

    def _publish(self, event, client, mode, result, *, status):
        owner = event.repository.owner
        repository = event.repository.name
