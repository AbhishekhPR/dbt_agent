...
A prebuilt history fixture is included at `.relium/deployment_history.json`, so the second command can be run directly to demonstrate the historical comparison. If your local review blocks the first deployment and you want to overwrite the fixture manually, add `--allow-blocked-recording` to the first command.
