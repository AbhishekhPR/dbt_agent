...
def capture(review_id, label, out_dir, *, app_url=APP_URL, expect_decision=None,
    ...
    with sync_playwright() as p:
        ...
        try:
            # ---- review list -------------------------------------------
            page.goto(f"{app_url}/reviews", wait_until="networkidle")
            page.wait_for_selector('[data-testid="review-table"]', timeout=20000)
            page.screenshot(path=str(out / f"{label}-01-review-list.png"),
                            full_page=full_page)

            # ---- review detail -----------------------------------------
            page.goto(f"{app_url}/reviews/{review_id}", wait_until="networkidle")
            page.wait_for_selector('[data-testid="decision-badge"]', timeout=20000)
            ...
            observed["decision_text"] = _text(page, '[data-testid="decision-badge"]')
            observed["lifecycle_state"] = _text(page, '[data-testid="lifecycle-state"]')
            observed["health"] = _text(page, '[data-testid="health"]')
            observed["coverage"] = _text(page, '[data-testid="coverage-badge"]')
            observed["finding_codes"] = [
                node.get_attribute("data-code")
                for node in page.query_selector_all('[data-testid="finding"]')
            ]
            ...
            # ---- publication -------------------------------------------
            publication = page.query_selector('#publication')
            if publication:
...
def _watch(page, expect_decision, seconds):
    ...
    try:
        page.wait_for_function(
            """(expected) => {
                 const el = document.querySelector('[data-testid="decision-badge"]');
...
def capture_incident(incident_id, out_dir, *, app_url=APP_URL):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            page.wait_for_selector('#rca', timeout=20000)
