...
class CliTests(unittest.TestCase):
    def test_top_level_help_excludes_stale_commands(self):
        ...
        self.assertTrue(
            {"simulate", "sql_metadata", "sql_risks", "freshness"}.isdisjoint(
                displayed_commands
            )
        )
    ...
    def test_history_aware_demo_loads_previous_snapshot_and_shows_semantic_diff(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            history_path.write_text(
                (DEMO_DIR / "fixtures" / "deployment_history.json").read_text(
                    encoding="utf-8"
                ),
                encoding="utf-8",
            )
    ...
    def test_history_aware_demo_prebuilt_history_fixture_supports_one_command_review(self):
        result, output = _invoke(
            [
                ...
                "fct_revenue",
                "--history-path",
                str(DEMO_DIR / "fixtures" / "deployment_history.json"),
                "--deployment-id",
                "current_deploy",
                "--format",
                "markdown",
